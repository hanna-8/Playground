...
func main() {
  ...
  rand.Seed(time.Now().UnixNano()) // ensures different values generated by rand on multiple program executions
  ...
  panic("show me the stack") // check whether there are still hanging / executing goroutines, except main
}
...
func printAll(conn *sql.DB, wg *sync.WaitGroup) {
  ...
  for rows.Next() {
    ...
    time.Sleep(time.Duration(rand.Intn(50)) * time.Millisecond) // highlight concurrency
  }
...
func compute(conn *sql.DB, wg *sync.WaitGroup) {
  ...
  time.Sleep(time.Duration(rand.Intn(100)) * time.Millisecond) // highlight concurrency
  fmt.Println("Avg:", avg)
